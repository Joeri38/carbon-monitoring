...
input.cent_x = 0;
input.cent_y = 0;
input.patterned_layer_t_range = 1.9e-6; % [m] membrane thickness
input.period_range = 0.44e-6; % [m] OG: 0.64e-6
input.staircase_N = 20;
res = 64;
input.diameter1_range = linspace(0.12e-6, 0.21e-6,res);
input.diameter2_range = 0.12e-6;  % [m] range of diameters from 0 to 2.5e-6 in 100 steps
...
save_data = true;
if save_data
    save("library/pillars - supercell/z_real_test_" + num2str(input.lambda0*1e6) + "um.mat", 'z_real');
    save("library/pillars - supercell/z_imag_test_" + num2str(input.lambda0*1e6) + "um.mat", 'z_imag');
end
...
subplot(1,2,1);
plot(input.diameter1_range*1e6,phase(1:res));
xlabel('Diameter 1 [um]');
ylabel('Phase [rad]');
title("Wavelength " + num2str(input.lambda0) + ", period " + num2str(input.period_range));
%ylim([0, 2.05]);

subplot(1,2,2);
plot(input.diameter1_range*1e6,transmission(1:res));
xlabel('Diameter 1 [um]');
ylabel('Transmission [%]');
title("Wavelength " + num2str(input.lambda0) + ", period " + num2str(input.period_range));
%ylim([0, 105]);
