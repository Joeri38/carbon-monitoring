...
input.cent_x = 0;
input.cent_y = 0;
input.patterned_layer_t_range = 3e-6; % [m] membrane thickness
input.period_range = 0.64e-6; % [m] OG: 0.9e-6
input.staircase_N = 20;
res = 5;
%input.diameter1_range = 0.100e-6;
%input.diameter2_range = 0.150e-6; 
%input.diameter3_range = 0.200e-6;
%input.diameter4_range = 0.250e-6;
input.diameter1_range = linspace(0.1e-6,0.3e-6,res);
input.diameter2_range = linspace(0.1e-6,0.3e-6,res); 
input.diameter3_range = linspace(0.1e-6,0.3e-6,res);
input.diameter4_range = linspace(0.1e-6,0.3e-6,res); % [m] range of diameters from 0 to 2.5e-6 in 100 steps
...
total_tic = tic;
[all_meta_atoms,completed_jobs] = get_supercell_meta_atom_GD(input);
...
%%
diameter1 = transpose(input.diameter1_range);
diameter2 = transpose(input.diameter2_range);
diameter3 = transpose(input.diameter3_range);
diameter4 = transpose(input.diameter4_range);
...
save_data = false;
if save_data
    save("library/supercell/thickness 3um/transmission_0.76um_res5.mat", 'transmission');
    save("library/supercell/thickness 3um/phase_0.76um_res5.mat", 'phase');
end
...
subplot(1,2,1);
%plot(input.diameter1_range*1e6,phase/pi);
%hold on;
...
subplot(1,2,2);
%plot(input.diameter1_range*1e6,100*transmission);
