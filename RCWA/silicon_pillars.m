...
input.period_range = 0.22e-6; % [m] OG: 0.64e-6
input.staircase_N = 20;

res = 100;
input.diameter_range = linspace(0.12e-6, 0.21e-6, res);
...
save_data = true;
if save_data
    save("library/pillars - single cell/z_real_" + num2str(input.lambda0*1e6) + "um.mat", 'z_real');
    save("library/pillars - single cell/z_imag_" + num2str(input.lambda0*1e6) + "um.mat", 'z_imag');
end
...
subplot(1,2,1);
%plot(input.diameter_range*1e6,phase(1:res)/pi);
...
saveas(gcf, "figs/single cell/lamb=" + num2str(input.lambda0*1e6) + "um_p=" + num2str(input.period_range*1e9) + "nm_t=" + num2str(input.patterned_layer_t_range*1e9) + "nm.fig");
