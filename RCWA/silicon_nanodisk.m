...
input.cent_x = 0;
input.cent_y = 0;
input.patterned_layer_t_range = 1.2e-6; % [m] membrane thickness
input.period_range = 0.25e-6; % [m] OG: 0.64e-6
input.staircase_N = 20;

res = 400;
input.diameter_range = linspace(0.01e-6, 0.22e-6, res);
...
subplot(1,2,2);
plot(input.diameter_range*1e6,100*transmission(1:res));
xlabel('Diameter 1 [um]');
ylabel('Transmission [%]');
title("Lambda " + num2str(input.lambda0*1e6) + "um, period " + num2str(input.period_range*1e9) + "nm, thickness " + num2str(input.patterned_layer_t_range*1e9) + "nm");
ylim([0, 105]);

saveas(gcf, "figs/lamb=" + num2str(input.lambda0*1e6) + "um_p=" + num2str(input.period_range*1e9) + "nm_t=" + num2str(input.patterned_layer_t_range*1e9) + "nm.fig");
