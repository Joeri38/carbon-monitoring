...
input.cent_x = 0;
input.cent_y = 0;
input.patterned_layer_t_range = 2e-6; % [m] membrane thickness
input.period_range = 0.32e-6; % [m] OG: 0.9e-6
input.staircase_N = 20; 
input.diameter_range = linspace(0.1e-6,0.3e-6,200); % [m] range of diameters from 0 to 2.5e-6 in 100 steps
input.lambda_range = 0.76e-6; % [m] 3.5 um
input.lambda0 = 0.76e-6; % [m] 3.5 um
input.debug = false;
input.multiplex = true;

[all_meta_atoms,completed_jobs] = get_circle_meta_atom_GD(input);
...
save_data = true;
if save_data
    save("library/holes - single cell/thickness 2um/z_real_" + num2str(input.lambda0*1e6) + "um.mat", 'z_real')
    save("library/holes - single cell/thickness 2um/z_imag_" + num2str(input.lambda0*1e6) + "um.mat", 'z_imag')
end
